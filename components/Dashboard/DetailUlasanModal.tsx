            ...
            <div className='p-4 sm:ml-64 flex justify-center bg-black bg-opacity-50'>
                <div className="bg-white dark:bg-[#1B262C] rounded shadow-md p-6 dark:text-white">
                    <div className='mb-4 flex justify-between'>
                        <h2 id="modal-title" className="text-xl font-medium">{ulasan.nama}</h2>
                    ...
                    <p id="modal-description" className="text-gray-600 mb-2">{ulasan.ulasan}</p>
                    <p className="text-gray-500 mb-4">{new Date(ulasan.waktu.seconds * 1000).toLocaleString()}</p>
                    ...
                    <div className="mb-4">
                        <h3 className="text-lg font-medium">Balasan</h3>
                        <div className="space-y-2 mt-2">
                            {balasanList.map((balasan) => (
                                <div key={balasan.id} className="bg-gray-100 dark:bg-gray-800 p-3 rounded-md">
                                    <p className="text-gray-800 dark:text-gray-200"><strong>{balasan.nama}:</strong> {balasan.balasan}</p>
                                    <p className="text-gray-500 text-sm">{new Date(balasan.waktu.seconds * 1000).toLocaleString()}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    <form onSubmit={handleBalasanSubmit}>
                        <div className="mb-4">
                            <textarea
                                ...
                                className="w-full p-2 border rounded-md bg-gray-50 dark:bg-[#1B262C] dark:border-gray-600 dark:text-white"
